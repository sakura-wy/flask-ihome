...
def save_order():
    ...
    try:
        ...
        # 此处计算预定天数， 应明确标识对天数的设定（如中午十二点之前不计入当天等）
    ...
    print(count)
    if count > 0:
        return jsonify(errno=RET.DATAERR, errmsg='房屋已被预订， 存在冲突')
    ...
    return jsonify(errno="OK", errmsg="OK", data={'order_id': order.id})
...
def get_user_orders():
    ...
    return jsonify(errno=RET.OK, errrmsg="OK", data={'orders': orders_dict_li})
