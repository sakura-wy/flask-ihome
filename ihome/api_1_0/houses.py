...
# GET /api/v1.0/houses?sd=20210712&ed=20210714&aid=10&sk=new&p=1
...
def get_house_list():
    ...
    if area_id:
        ...
        # 设置缓存
        redis_key = 'house_list_{}_{}_{}_{}'.format(start_date, end_date, area, sort_key)
        try:
        ...
        # 查询冲突订单， 提取冲突订单中的房屋id
        # 此处， 直接消除条件即设置为无限
        # 1. 当结束时间不存在时，可设置结束时间为起始时间用一天，也可设置为无限（查询只队订单的结束时间做出了限制，无论何时起始都冲突）
        # 2.  当起始时间不存在时，可以设置起始时间为今天， 也可设置为无限（即只对订单的起始时间作出限制，无论何时结束都冲突）
